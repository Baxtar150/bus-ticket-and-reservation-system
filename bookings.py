...
def booked():
    ...
    if((Bus_id.get()) == "" or (Boarding.get()) == "" or (Destination.get())== "" or (Busstop.get())== "" or (Passengerno.get())== ""):
    ...
    # print(values)
    try:
        sql = "INSERT INTO `commuter`(`bus_id`, `boarding`, `destination`, `bus_stop`, `passenger_no`,`Ticket_no`) VALUES (%s,%s,%s,%s,%s,%s)"
        print("i dey here")
        
        values=((Bus_id.get()),(Boarding.get()),(Destination.get()),(Busstop.get()),(Passengerno.get()),Ticketno.get())
        print("i dey here")
        print(values)
...
busno_Combo = ttk.Combobox(frame_Booking_Panel, state='readonly',values=bus_no,textvariable=Bus_id,font=("Arial",10,"bold"))
busno_Combo.place(relx=0.19, rely=0.18, relheight=0.05, relwidth=0.2)
# busno_Combo.set("Select Number")

doblbl=ttk.Label(frame_Booking_Panel,text="Date",style="textlbl.TLabel")
doblbl.place(relx=0.01,rely=0.28)

dobentry=DateEntry(frame_Booking_Panel)
dobentry.place(relx=0.19,rely=0.27)

boarding_lbl = ttk.Label(frame_Booking_Panel,text="Boarding :",style="textlbl.TLabel")
boarding_lbl.place(relx=0.01, rely=0.38, height=23, width=120)
...
destination_lbl.place(relx=0.52, rely=0.38, height=21, width=134)
...
boarding_Combo.place(relx=0.19, rely=0.38, relheight=0.05, relwidth=0.2)
...
des_Combo.place(relx=0.74, rely=0.38, relheight=0.05, relwidth=0.2)
...
des_bb_Combo .place(relx=0.74, rely=0.40, relheight=0.05, relwidth=0.2)
